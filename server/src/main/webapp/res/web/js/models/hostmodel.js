...
window.Host = Backbone.Model.extend({
    ...
    initialize:function () {
    	var self=this;
    	self.set({'name':getHostName(self.id)});
    	$.ajax({
    		url:hackBase+"getLongAddress",
    		data:{'macAddress':self.id},
    		dataType:'json',
    		success:function(d){
    			console.log("rules size is "+rules.length);
    			_.each(rules,function(t){
    				if(t.dl_src===d){
    					console.log(self.id+":"+t.action);
    					self.set({'httpPortStatus':t.action});
    				}
    			});
    		}
    	});
    	
    	$.get("/nms/getVendor?mac="+self.id,function(data){
    		var v;
    		if(data) {
    			v=data.vendor;
    		}else{
